...
expanded_data = pd.read_csv('count_differences1.csv')

# Extract the relevant features and target
X = expanded_data[['is_counts_equal', 'num_dice', 'num_sides','count_difference','num_zeros']]
y_string = expanded_data['string']

# Perform train-test split
X_train, X_test, y_string_train, y_string_test = train_test_split(
    X, y_string, random_state=42)
...
model = MLPRegressor(hidden_layer_sizes=(150, 150, 150, 150, 150), activation='relu',max_iter=300, random_state=42)
...
new_data = pd.DataFrame({'is_counts_equal': [True], 'num_dice': [5], 'num_sides': [30], 'count_difference':[0], 'num_zeros':[7140]})
