...
while whilecount<2:
    ...
    for c in alphabetic_string:
        ...
        for k in keys:
            if k.count(c) == 0:
                ...
                if len(k2) == 2:
                    count_2[k2] = count.get(k2, 0) + count[k]
                elif len(k2) == 3:
                    count_3[k2] = count.get(k2, 0) + count[k]
                elif len(k2) == 4:
                    count_4[k2] = count.get(k2, 0) + count[k]

    # Print counts with length 5 and check if they are all equal ##ADD WEAVE METHOD, SAME BUT FOR 2,3,4 
    ...
    # Check if counts for lengths 2, 3, and 4 are equal
    if len(set(count_2.values())) == 1 and len(set(count_3.values())) == 1 and len(set(count_4.values())) == 1:
        # Save data to a CSV file
        for k, v in count.items():
            if len(k) == 5:
                training_data.append([v,k, alphabetic_string, is_counts_equal,num_dice,num_sides])

        with open('data1.csv', 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['counts', 'subsequence', 'string', 'is_counts_equal','num_dice','num_sides'])
